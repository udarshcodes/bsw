...
from qiskit import QuantumCircuit              # Build single-qubit circuits for gates
from qiskit.quantum_info import Operator       # Extract unitary matrices from circuits
import requests                                 # HTTP client to call Gemini API
...
def state_to_bloch(state_vector):
    ...
    Uses ρ = |ψ⟩⟨ψ| and computes expectation values of Pauli matrices.
...
def apply_gate_to_state(theta_deg, phi_deg, gate_name):
    ...
    Builds |ψ⟩ from spherical angles, applies the requested gate via Qiskit's
    unitary, then converts the resulting state back to (θ, φ) for UI use.
    ...
    # Build a 1-qubit circuit and map human-friendly gate names to circuit methods
    ...
    # Apply gate if recognized; otherwise leave state unchanged
    if gate_name in gate_map:
        gate_map[gate_name](0)                    # Append gate to qubit 0
        gate_operator = Operator(gate_circuit)    # Get unitary matrix U
        new_state_vector = gate_operator.data @ current_state_vector  # |ψ'⟩ = U|ψ⟩
    else:
        new_state_vector = current_state_vector   # No-op on unknown gate

    # Convert the new state back into spherical angles
    x, y, z = state_to_bloch(new_state_vector)    # Bloch components
    new_theta_rad = np.arccos(np.clip(z, -1, 1))  # θ = arccos(z), clipped for numerical stability
    new_phi_rad = np.arctan2(y, x)                # φ = atan2(y, x)

    # Return degrees, wrapping φ to [0, 360)
...
def create_figure_for_state(theta_deg, phi_deg):
    """Create a Plotly 3D figure of the Bloch sphere and the current state.

    Adds: translucent sphere, latitude/longitude grid, XYZ axes, arrow + cone
    showing the qubit direction, and labels for |0⟩ and |1⟩ at the poles.
    ...
    # Translucent sphere surface (no colorbar)
    fig.add_trace(
        go.Surface(
    ...
    # Latitude lines (constant z), sweep t around a circle for each elevation i
    ...
    # --- NEW: Add |0⟩ and |1⟩ labels directly on the poles ---
    ...
    # Layout: equal aspect ratio, dark theme; hide tick labels on x/y/z for a cleaner look
    fig.update_layout(
        ...
        scene=dict(
            ...
            # --- MODIFIED: Removed tickvals/ticktext; hide z tick labels ---
...
def get_ai_explanation(state_data, last_action):
    ...
    Reads an API key from a secret file, validates it, builds a system/user
    prompt pair (with strict formatting rules), calls Gemini's
    `generateContent` endpoint, and returns the model's text response. Includes
    guardrails and friendly error messages.

    Args:
        state_data (dict): Contains angles, formatted state string, and probabilities.
        last_action (str): Description of the user's last interaction.

    Returns:
        str: Markdown-formatted explanation text, or an error message.
    """
    api_key = ""  # Will be set if secret file exists
    secret_path = '/etc/secrets/GEMINI_API_KEY'  # Render-style secret mount path

    # Try to read API key from the secret file if present
    if os.path.exists(secret_path):
        with open(secret_path, 'r') as f:
            api_key = f.read().strip()

    # If no key is configured, instruct operators how to enable the feature
    if not api_key:
        return (
    ...
    # Lightweight key-format sanity check (Gemini keys typically start with 'AIza')
